    ...
    <div class="col-sm-6 col-xl-4">
        <div class="course-style1">
            ...
            <div class="course-content">
                <h3 class="h5 course-name"><a href="{{route('frontend.course',$course->id)}}">{{ $course->title }}</a></h3>
                <div class="course-teacher"><a href="#" class="text-inherit">بواسطة : {{ $course->center?->name }}</a></div>
            </div>
            <div class="course-meta">
                <span><i class="fal fa-users"></i>{{ $course->beneficiary_count }} مستفيد</span>
                <span><i class="fal fa-calendar-alt"></i>{{ \Carbon\Carbon::parse($course->date)->format('d M Y') }}</span>
            </div>
        </div>
    </div>
    @empty
